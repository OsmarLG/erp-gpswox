                                ...
                                <tbody>
                                    @foreach ($servicesPending as $service)
                                        <tr>
                                            ...
                                            <td>
                                                <x-button wire:click="initService({{ $service->id }})" label="Ver" class="btn-primary btn-sm" />
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                                ...
                                <tbody>
                                    @foreach ($servicesCompleted as $service)
                                        <tr>
                                            ...
                                            <td>{{ $service->vehicle->nombre_unidad ?? 'N/A' }}</td>
                                            <td>{{ $service->operador->name ?? 'N/A' }}</td>
                                            <td>{{ $service->fecha_realizacion ? $service->fecha_realizacion->format('d-m-Y') : 'Sin fecha' }}</td>
                                            <td>{{ $service->created_at->format('d-m-Y') }}</td>
                                            <td>{{ $service->created_at->diffForHumans() }}</td>
                                            <td>{{ ucfirst($service->status) }}</td>
                                            <td>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($service->status === 'initiated')
                                                <x-button wire:click="initService({{ $service->id }})" icon="o-cog" label="Continuar" class="btn-primary btn-sm" />
                                            @else
                                                <x-button wire:click="initService({{ $service->id }})" icon="o-cog" label="Iniciar" class="btn-primary btn-sm" />
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                            ...
                            <tbody>
                                @foreach ($vehicleRequestsPending as $service)
                                    <tr>
                                        ...
                                        <td>
                                            @if($service->status === 'initiated')
                                                <x-button wire:click="showRequest({{ $service->id }})" icon="o-cog" label="Continuar" class="btn-primary btn-sm" />
                                            @else
                                                <x-button wire:click="initRequest({{ $service->id }})" icon="o-cog" label="Iniciar" class="btn-primary btn-sm" />
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
